...
#[derive(Debug)]
pub enum Outcome {
    /// Setup mode picked an account. Parent should persist the descriptor
    /// and enter the dashboard with the live signer.
    SetupComplete {
        account: AccountDescriptor,
        signer: KaoSigner,
    },
    /// Reconnect mode finished. Parent enters the dashboard; no save needed.
    ReconnectComplete { signer: KaoSigner },
    /// User backed out (setup mode → return to setup-method picker;
    /// reconnect mode → return to unlock screen).
    Back,
}
...
pub enum Mode {
    Setup,
    /// Reconnect to a previously-registered Ledger account. `expected_address`
...
enum Stage {
    ...
    /// Setup-mode: user has picked, awaiting signer construction.
    Building {
        picked: u32,
    },
    Error(String),
...
#[derive(Debug)]
pub struct ConnectLedgerScreen {
    mode: Mode,
    stage: Stage,
    network: Arc<dyn BalanceFetcher>,
}

impl ConnectLedgerScreen {
...
fn picking_card<'a>(t: KaoTheme, rows: &'a [PickRow]) -> Element<'a, Message> {
    ...
    let body = column![
        kao_hero(t, "(•̀ᴗ•́ )و", 52.0),
        vspace(10),
        screen_title(t, "Choose a Ledger Account"),
        vspace(6),
        screen_subtitle(
            t,
            "Pick which Ledger Live account to register (m/44'/60'/N'/0/0)."
        ),
        vspace(20),
        header,
        vspace(4),
...
fn building_card<'a>(t: KaoTheme, picked: u32) -> Element<'a, Message> {
    let body = column![
        kao_hero(t, "(￣ω￣)", 56.0),
        vspace(12),
        screen_title(t, "Confirm on your Ledger"),
        vspace(6),
        screen_subtitle(
            t,
        ...
        vspace(18),
        container(
            text(format!("HD account #{picked} (m/44'/60'/{picked}'/0/0)"))
                .size(12)
                .color(t.sub)
                .font(mono())
        )
