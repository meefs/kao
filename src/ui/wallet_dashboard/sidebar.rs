//! Vertical sidebar — wordmark, nav icons, theme dots. Stateless view.

use iced::border::Radius;
use iced::widget::{Space, column, container, mouse_area, text};
use iced::{Alignment, Background, Border, Color, Element, Length, Padding};

use crate::ui::kao_theme::{KaoTheme, ThemeKind, with_alpha};
use crate::ui::kao_widgets::{bold, kao_fit};

use super::{Message, Nav};

pub fn view<'a>(t: KaoTheme, nav: Nav, theme_kind: ThemeKind) -> Element<'a, Message> {
    ...
    for k in ThemeKind::ALL {
        nav_col = nav_col.push(theme_dot(t, theme_kind, k));
    }
    nav_col = nav_col.push(Space::new().height(6));
...
fn theme_dot<'a>(t: KaoTheme, current: ThemeKind, k: ThemeKind) -> Element<'a, Message> {
    let selected = current == k;
    let swatch = k.swatch();
    let outline_col = if selected { t.text } else { Color::TRANSPARENT };
    let dot = container(Space::new().width(36).height(36))
