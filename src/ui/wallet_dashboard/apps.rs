...
impl AppsPane {
    ...
    pub fn view<'a>(
        ...
        let scroller = scrollable(container(content).padding(Padding::from([28, 32])))
            .width(Length::Fill)
            .height(Length::Fill)
    ...
    fn swap_view<'a>(
        ...
        // Orders header: label + a fetch button (only when there are orders).
        let mut orders_header = row![text("YOUR ORDERS").size(11).color(t.sub).font(mono_bold())]
            .align_y(Alignment::Center)
            .width(Length::Fill);
        if !orders.is_empty() {
            orders_header = orders_header.push(Space::new().width(Length::Fill));
            orders_header = orders_header.push(fetch_button(t));
        }
        ...
        if orders.is_empty() {
            content = content.push(
                text("No orders yet — place a swap above.")
                    .size(12)
                    .color(t.sub),
            );
        } else {
