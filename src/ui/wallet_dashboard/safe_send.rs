...
use crate::ui::kao_widgets::{
    avatar, black, bold, colored_address, hover_tint, kao_fit, kao_fit_size, kao_scrollable_style,
    kao_text, modal_wrapper, mono, mono_bold, primary_button, review_row, secondary_button,
...
impl SafeSendPane {
    pub fn new(safe: &SafeDescriptor, accounts: &[AccountDescriptor]) -> Self {
        ...
        for &idx in &safe.linked_signer_indices {
            match accounts.get(idx as usize) {
                ...
                Some(AccountDescriptor::Ledger { .. }) => unsupported_owner_kinds.push("Ledger"),
                Some(AccountDescriptor::Trezor { .. }) => unsupported_owner_kinds.push("Trezor"),
    ...
    fn can_confirm(&self) -> bool {
        !self.busy && self.last_tx_hash.is_none() && self.can_continue_from_compose()
    }
    ...
    pub fn update(&mut self, msg: Message) -> (Task<Message>, Option<Outcome>) {
        match msg {
            ...
            Message::Step(n) => {
                match n {
                    0 => self.step = 0,
                    1 if self.can_continue_from_compose() => self.step = 1,
                    _ => {}
                }
                (Task::none(), None)
            }
            Message::Confirm => {
                // The dashboard intercepts Confirm to spawn the
                // broadcast task; we only land here if it didn't.
                (Task::none(), None)
            }
    ...
    pub fn view<'a>(
        ...
        if !self.has_enough_local_signers() {
            let body = column![
                step_header(t, 0),
                progress_bar(t, 0),
                vspace(20),
    ...
    fn view_review<'a>(&'a self, t: KaoTheme, recipient_name: Option<String>) -> Element<'a, Message> {
        ...
        let mut confirm_btn = primary_button(
            t,
            if self.busy { "Signing & sending…" } else { "Confirm Send ✓" },
            can_confirm,
        );
        if can_confirm {
            confirm_btn = confirm_btn.on_press(Message::Confirm);
        }
        let back_btn = secondary_button(t, "← Back").on_press(Message::Step(0));
        let action_row = row![
            container(back_btn).width(Length::FillPortion(1)),
            Space::new().width(9),
            container(confirm_btn).width(Length::FillPortion(3)),
        ]
        .align_y(Alignment::Center);
