//! kao-vendored fork of op-alloy-network 0.20.0. The only delta from
//! upstream is the deletion of `impl NetworkWallet<Optimism> for
//! EthereumWallet` (originally lib.rs:202-239 upstream). That impl
//! collides with alloy-network 1.x's blanket
//! `impl<N: Network> NetworkWallet<N> for EthereumWallet`, which Rust's
//! coherence rule rejects (E0119) once both crates land in the same build
//! graph (they do here, via helios-opstack -> op-alloy-network and our
//! direct alloy = "2" / helios's transitive alloy 1.x deps).
//!
//! The blanket already provides `NetworkWallet<Optimism>` for
//! `EthereumWallet` for any caller that needs it. The wallet itself never
//! signs Optimism transactions through this trait — helios-opstack uses it
//! purely for verified balance reads — so the vendor delta is invisible at
//! runtime.
...
// NOTE: upstream defines `impl NetworkWallet<Optimism> for EthereumWallet`
// here. We removed it; see the module-level comment for why.
